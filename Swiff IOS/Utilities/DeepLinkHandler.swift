...
class DeepLinkHandler: ObservableObject {
    ...
    private func process(_ deepLink: DeepLinkType) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            ...
            case .viewSubscriptions:
                self.selectedTab = 1 // Subscriptions tab

            case .viewAnalytics:
                self.selectedTab = 3 // Analytics tab (assuming tab order)

            case .viewSubscription(let id):
                self.selectedSubscriptionId = id
                self.selectedTab = 1 // Navigate to subscriptions tab
...
struct DeepLinkHelper {
    ...
    static func makeURL(for type: DeepLinkType) -> URL? {
        ...
        case .viewAnalytics:
            return URL(string: "swiff://action/view-analytics")
        case .viewSubscription(let id):
...
extension DeepLinkHelper {
    static let examples: [(name: String, url: URL?)] = [
        ...
        ("View Analytics", makeURL(for: .viewAnalytics)),
