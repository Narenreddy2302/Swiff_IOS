...
struct ErrorEvent: Codable, Identifiable {
    ...
    static func from(_ error: ApplicationError, category: String = "General") -> ErrorEvent {
        ...
        let deviceInfoString =
            "\(deviceInfo.model) - iOS \(deviceInfo.systemVersion) - \(deviceInfo.locale)"

        // Convert additionalInfo [String: Any] to [String: String]
        let metadata = error.context.additionalInfo.reduce(into: [String: String]()) {
            result, pair in
...
struct ErrorStatistics: Codable {
    ...
    var summary: String {
        ...
        if let oldest = oldestErrorDate, let newest = newestErrorDate {
            lines.append(
                "Date Range: \(oldest.formatted(date: .abbreviated, time: .omitted)) - \(newest.formatted(date: .abbreviated, time: .omitted))"
            )
...
class ErrorAnalytics {
    ...
    func trackStandardError(
        _ error: Error, category: String = "General", severity: ErrorSeverity = .error
    ) {
        let appVersion =
    ...
    func getStatistics(for period: DateInterval? = nil) -> ErrorStatistics {
        ...
        return ErrorStatistics(
            ...
            recentErrors: filteredEvents.filter { $0.timestamp > Date().addingTimeInterval(-86400) }
                .count,
    ...
    func generateReport(for period: DateInterval? = nil) -> ErrorReport {
        ...
        if stats.totalErrors > 1000 {
            recommendations.append(
                "High error volume detected. Review and address top error patterns.")
        }
        ...
        for pattern in patterns.prefix(5) {
            if pattern.severity == "Critical" || pattern.severity == "Fatal" {
                recommendations.append(
                    "Investigate recurring \(pattern.errorType) (\(pattern.occurrences) occurrences)"
                )
            }
        }
        ...
        if stats.affectedUsers > 100 {
            recommendations.append(
                "Errors affecting \(stats.affectedUsers) users. Prioritize fixes for user-facing issues."
            )
        }
    ...
    func exportEvents(format: ExportFormat = .json, period: DateInterval? = nil) -> URL? {
        ...
        do {
            ...
            case .csv:
                var csv =
                    "ID,Timestamp,Error Type,Code,Domain,Severity,Message,Category,Device,Version\n"
                for event in filteredEvents {
                    csv +=
                        "\(event.id),\(event.timestamp),\(event.errorType),\(event.errorCode),\(event.errorDomain),\(event.severity),\"\(event.message)\",\(event.category),\(event.deviceInfo),\(event.appVersion)\n"
                }
            ...
            logger?.info(
                "Exported \(filteredEvents.count) events to \(format.fileExtension.uppercased())",
                category: "Analytics")
            return exportURL

        } catch {
            logger?.error(
                "Failed to export events: \(error.localizedDescription)", category: "Analytics")
            return nil
        }
    }
    ...
    func exportReport(_ report: ErrorReport) -> URL? {
        ...
        do {
            ...
            logger?.error(
                "Failed to export report: \(error.localizedDescription)", category: "Analytics")
            return nil
        }
    }
    ...
    func performCleanup() {
        let cutoffDate = Date().addingTimeInterval(
            -TimeInterval(configuration.retentionDays * 86400))
    ...
    func getStorageSize() -> Int64 {
        guard
            let files = try? FileManager.default.contentsOfDirectory(
                at: storageURL, includingPropertiesForKeys: [.fileSizeKey])
        else {
            return 0
        }
    ...
    private func loadEvents() {
        ...
        do {
            ...
            logger?.error(
                "Failed to load events: \(error.localizedDescription)", category: "Analytics")
        }
    }
    ...
    private func saveEvents() {
        ...
        do {
            ...
            logger?.error(
                "Failed to save events: \(error.localizedDescription)", category: "Analytics")
        }
    }
