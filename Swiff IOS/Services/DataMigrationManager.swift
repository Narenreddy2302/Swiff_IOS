...
// MARK: - Migration Result (renamed to avoid conflict with DataMigrationService)

struct DataMigrationManagerResult {
    let success: Bool
    let fromVersion: Int
    let toVersion: Int
...
class DataMigrationManager {
    ...
    func migrate() async throws -> DataMigrationManagerResult {
        let currentVersion = getCurrentVersion()
        return try await migrate(to: Self.currentVersion, from: currentVersion)
    }

    /// Perform migration to specific version
    func migrate(to targetVersion: Int, from sourceVersion: Int? = nil) async throws -> DataMigrationManagerResult {
        ...
        do {
            ...
            return DataMigrationManagerResult(
                success: true,
