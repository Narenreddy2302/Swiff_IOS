...
struct SubscriptionBillingSummaryCard: View {
    ...
    @Binding var enableRenewalReminder: Bool
    ...
    private var membershipDuration: String {
        let components = Calendar.current.dateComponents([.month, .day], from: subscription.createdDate, to: Date())
    ...
    // MARK: - Compact Reminder Toggle

    private var compactReminderToggle: some View {
        VStack(spacing: 0) {
            Divider()
                .padding(.bottom, 16)
            ...
            HStack(spacing: 12) {
                // Bell icon
                Image(systemName: enableRenewalReminder ? "bell.fill" : "bell.slash")
                    .font(.system(size: 18))
                    .foregroundColor(enableRenewalReminder ? .wiseBlue : .wiseSecondaryText)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if enableRenewalReminder {
                        Text("\(reminderDaysBefore) day\(reminderDaysBefore == 1 ? "" : "s") before")
                            .font(.spotifyCaptionMedium)
                            .foregroundColor(.wiseSecondaryText)
                    }
                }
                ...
                // Days selector (only visible when enabled)
                if enableRenewalReminder {
                    Menu {
                ...
                Toggle("", isOn: $enableRenewalReminder)
                    .labelsHidden()
                    .tint(.wiseBlue)
                    .onChange(of: enableRenewalReminder) { _, _ in
                        onReminderChanged()
                    }
            }
        }
...
#Preview("Active Subscription") {
    ScrollView {
        SubscriptionBillingSummaryCard(
            ...
            enableRenewalReminder: .constant(true),
            reminderDaysBefore: .constant(3),
            onReminderChanged: {}
        )
    }
