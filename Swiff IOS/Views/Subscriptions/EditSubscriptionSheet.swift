...
struct EditSubscriptionSheet: View {
    ...
    init(subscription: Subscription, onSubscriptionUpdated: @escaping () -> Void) {
        ...
        _trialEndDate = State(
            initialValue: subscription.trialEndDate ?? Calendar.current.date(
                byAdding: .day, value: 7, to: Date()) ?? Date())
        _willConvertToPaid = State(initialValue: subscription.willConvertToPaid)
        _priceAfterTrial = State(
            initialValue: subscription.priceAfterTrial != nil
                ? String(format: "%.2f", subscription.priceAfterTrial!) : "")
    ...
    private var isTrialExpiringSoon: Bool {
        ...
        let daysUntilEnd =
            Calendar.current.dateComponents([.day], from: Date(), to: trialEndDate).day ?? 0
    ...
    private func calculateTrialDuration() -> Int {
        let components = Calendar.current.dateComponents(
            [.day], from: trialStartDate, to: trialEndDate)
                                    ...
                                    ForEach(availableIcons, id: \.self) { icon in
                                        Button(action: {
                                            ...
                                            Image(systemName: icon)
                                                ...
                                                .background(
                                                    selectedIcon == icon
                                                        ? Color(hexString: selectedColor).opacity(
                                                            0.1)
                                        ...
                                        Circle()
                                            ...
                                            .overlay(
                                                Circle()
                                                    .stroke(
                                                        Color.white,
                                                        lineWidth: selectedColor == colorHex ? 3 : 0
                                                    )
                                            )
                                            .shadow(
                                                color: Color.wiseShadowColor,
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        if isFreeTrial {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                DatePicker(
                                    "", selection: $trialStartDate, displayedComponents: .date
                                )
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                DatePicker(
                                    "", selection: $trialEndDate, in: trialStartDate...,
                            ...
                            Toggle(isOn: $willConvertToPaid) {
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Convert to Paid After Trial")
                                        ...
                                        .foregroundColor(.wisePrimaryText)
                                    Text(
                                        "Automatically switch to paid subscription when trial ends"
                                    )
                                    .font(.spotifyCaptionMedium)
                                    .foregroundColor(.wiseSecondaryText)
                                }
