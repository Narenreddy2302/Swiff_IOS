...
struct TransactionGroupHeader: View {
    ...
    private var dateLabel: String {
        ...
        let daysDifference =
            calendar.dateComponents([.day], from: transactionDay, to: today).day ?? 0
        ...
        case 1:
            return "Yesterday"
        default:
            // Format as "January 16" or full date
            let formatter = DateFormatter()
            if daysDifference < 7 {
                formatter.dateFormat = "EEEE"  // "Monday", "Tuesday", etc.
            } else {
                formatter.dateFormat = "MMMM d"  // "January 16"
            }
...
extension Array where Element == Transaction {
    ...
    func groupedByDateSections() -> [(
        sectionDate: Date, label: String, transactions: [Transaction]
    )] {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        ...
        return groups.map {
            (sectionDate: $0.key, label: sectionLabel(for: $0.key), transactions: $0.value)
        }
