...
struct UserProfileEditView: View {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    Button(action: saveProfile) {
                        ...
                        .padding(.vertical, 16)
                        .background(hasChanges ? Color.wiseForestGreen : Color.wiseSecondaryText.opacity(0.5))
                        .cornerRadius(12)
                    }
                    .disabled(!hasChanges)
                    .padding(.horizontal, 20)
    ...
    private func updateInitialsAvatar() {
        ...
        if components.count >= 2 {
            ...
            selectedAvatarType = .initials("\(first)\(last)".uppercased(), colorIndex: currentColorIndex)
        } else if let first = components.first, !first.isEmpty {
            selectedAvatarType = .initials(String(first.prefix(2)).uppercased(), colorIndex: currentColorIndex)
        }
    }
}
    ...
    var body: some View {
        ...
        .alert("Photo Error", isPresented: $showingPhotoError) {
            if let recovery = photoErrorRecovery {
                ...
                case .suggestCompression, .suggestConversion, .suggestSmallerImage, .suggestDifferentPhoto:
                    Button("OK") {}
                }
            } else {
                Button("OK") {}
                            ...
                            Text(emoji)
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 8)
                                        .fill(selectedEmoji == emoji ? Color.wiseForestGreen.opacity(0.2) : Color.clear)
                                )
                        }
                    }
                }
